...
class MotorBackendApiCategoryTreeTest extends TestCase
{
    ...
    public function can_show_a_single_category_tree()
    {
        ...
        $this->json('GET',
            '/api/category_trees/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(200)->seeJson([
            'name' => $record->name
        ]);
    }
    ...
    public function fails_to_show_a_single_category_tree_without_permission()
    {
        $record = create_test_category_tree();
        $this->json('GET',
            '/api/category_trees/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(403)->seeJson([
            'error' => 'Access denied.'
        ]);
    }
    ...
    public function can_search_for_a_category_tree()
    {
        ...
        $records = create_test_category_tree(10);
        $this->json('GET',
            '/api/category_trees?api_token=' . $this->user->api_token . '&search=' . $records[2]->name)->seeStatusCode(200)->seeJson([
            'name' => $records[2]->name
        ]);
    }
    ...
    public function can_show_a_second_category_tree_results_page()
    {
        ...
        $this->json('GET',
            '/api/category_trees?api_token=' . $this->user->api_token . '&page=2')->seeStatusCode(200)->seeJson([
            'current_page' => 2
        ]);
    }
    ...
    public function fails_if_trying_to_modify_a_category_tree_without_payload()
    {
        ...
        $record = create_test_category_tree();
        $this->json('PATCH',
            '/api/category_trees/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(422)->seeJson([
            'name' => [ 'The name field is required.' ]
        ]);
    }
