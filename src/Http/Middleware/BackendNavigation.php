...
class BackendNavigation
{
    ...
    {
        \Menu::make('', static function () {}); //  <-- we need to do this to load the deferred service provider as we're using our motor-backend class for the menu
        ...
        $menu->make('backendNavigation', static function ($menu) {
            ...
            foreach ($items as $item) {
                $menu->add(trans($item['name']), [
                    ...
                    ->nickname($item['slug'])
                    ->prepend('<i class="c-sidebar-nav-icon '.$item['icon'].'"></i>');
                // ])->nickname($item['slug'])->prepend('<i class="nav-icon ' . $item['icon'] . '"></i> <span>')->append('</span>');

                if (isset($item['items'])) {
                    $menu->get($item['slug'])
                        ->append(config('motor-backend-navigation.collapseIcon'));
